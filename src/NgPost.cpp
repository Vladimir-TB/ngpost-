...
#define NGPOST_STR_HELPER(x) #x
#define NGPOST_STR(x) NGPOST_STR_HELPER(x)

const char *NgPost::sAppName          = "ngPost";
const QString NgPost::sDisplayName    = "NgPost +";
const QString NgPost::sVersion        = QString::fromLatin1(NGPOST_STR(APP_VERSION));

#undef NGPOST_STR
#undef NGPOST_STR_HELPER
...
const QMap<NgPost::Opt, QString> NgPost::sOptionNames =
{
    ...
    {Opt::HELP,           "help"},
    {Opt::LANG,           "lang"},
    {Opt::THEME_COLOR,    "theme_color"},
    {Opt::THEME_MODE,     "theme_mode"},
    {Opt::UI_SCALE,       "ui_scale"},
    {Opt::QUICK_ADVANCED_VISIBLE, "quick_advanced_visible"},
    {Opt::AUTO_ADVANCED_VISIBLE,  "auto_advanced_visible"},
    {Opt::QUICK_POST_ARCHIVE_PASSWORD, "quick_post_archive_password"},
    {Opt::VERSION,        "version"},
    {Opt::CONF,           "conf"},
    ...
    {Opt::OBFUSCATE_FILE_NAME, "obfuscate_file_name"},
...
const QList<QCommandLineOption> NgPost::sCmdOptions = {
    ...
    {{"c", sOptionNames[Opt::CONF]},          tr( "use configuration file (if not provided, we try to load the user configuration file and migrate an old local ngPost.conf when found)"), sOptionNames[Opt::CONF]},
...
NgPost::NgPost(int &argc, char *argv[]):
    ...
    _lang("nl"), _themeColor(), _themeMode(), _uiScale(1.0),
    _quickAdvancedVisible(false), _autoAdvancedVisible(false), _quickPostArchivePassword(false), _translators(),
    ...
    _logFile(nullptr), _logStream(nullptr)
{
    ...
    _app->setApplicationName(QString::fromLatin1(sAppName));

#ifdef __USE_HMI__
    if (_hmi)
        _hmi->setWindowTitle(QString("%1 v%2").arg(sDisplayName).arg(sVersion));
#endif
...
QString NgPost::_findBundledArchiver() const
{
    const QString appDir = qApp->applicationDirPath();
    QStringList candidates;
#if defined(WIN32) || defined(__MINGW64__)
    candidates << "7za.exe" << "7zz.exe" << "7zr.exe" << "7z.exe" << "rar.exe";
#else
    candidates << "7zz" << "7za" << "7zr" << "7z" << "rar";
#endif

    for (const QString &candidate : candidates)
    {
        QFileInfo fi(QString("%1/%2").arg(appDir, candidate));
        if (fi.exists() && fi.isFile() && fi.isExecutable())
...
void NgPost::_detectBundledArchiver()
{
    const QString bundledArchiver = _findBundledArchiver();
    if (bundledArchiver.isEmpty())
        return;

    if (!_rarPath.isEmpty() && _rarPath != bundledArchiver)
        _log(tr("Configured archiver '%1' is overridden by bundled archiver '%2' next to ngPost.exe").arg(
                 _rarPath, QFileInfo(bundledArchiver).fileName()));

    _rarPath = bundledArchiver;
}
...
void NgPost::onRefreshprogressbarBar()
{
...
#endif
    {
        ...
        if (nbArticlesTotal != 0)
            progressbar = static_cast<float>(nbArticlesUploaded) / nbArticlesTotal;
...
void NgPost::_loadTanslators()
{
    ...
    if (_lang != "en"){
        ...
        else
        {
            ...
            _lang = fallbackLang;
            if (_lang != "en" && _translators.contains(_lang))
                _app->installTranslator(_translators[_lang]);
            else if (_lang != "en")
                _lang = "en";
        }
...
{
    _app->removeTranslator(_translators.value(_lang, nullptr));
    QString nextLang = lang.toLower();
    if (!_translators.contains(nextLang))
    {
        _cerr << tr("ERROR: couldn't find translator for lang %1").arg(nextLang) << "\n" << MB_FLUSH;
        nextLang = "nl";
        if (!_translators.contains(nextLang))
            nextLang = "en";
    }
    ...
    if (_translators.value(_lang, nullptr))
        _app->installTranslator(_translators[_lang]);
}
...
void NgPost::onCheckForNewVersion()
{
    ...
    QRegularExpression appVersionRegExp("^DEFINES \\+= \"APP_VERSION=\\\\\"((\\d+)\\.(\\d+)(?:\\.(\\d+))?)\\\\\"\"$");
    QStringList v = sVersion.split(".");
    int currentMajor = v.value(0).toInt();
    int currentMinor = v.value(1).toInt();
    int currentPatch = v.value(2).toInt();
    while (!reply->atEnd())
    {
        ...
        if (match.hasMatch())
        {
            ...
            int lastMajor = match.captured(2).toInt();
            int lastMinor = match.captured(3).toInt();
            int lastPatch = match.captured(4).toInt();
...
{
    if (_debug)
        _log(QString("Shutdown proc exitCode: %1").arg(exitCode));
...
bool NgPost::parseCommandLine(int argc, char *argv[])
{
...
#ifdef __DEBUG__
                qDebug() << "NNTP Server: " << user << ":***"
                         << "@" << host << ":" << port << ":" << nbCon << ":" << ssl;
#endif
                NntpServerParams *server = new NntpServerParams(host,
                                                                port,
                                                                auth,
                                                                user.toUtf8().toStdString(),
                                                                pass.toUtf8().toStdString(),
                                                                nbCon,
                                                                ssl);
                _nntpServers << server;
            }
    ...
    if (parser.isSet(sOptionNames[Opt::HOST]))
    {
        ...
        {
            server->auth = true;
            server->user = parser.value(sOptionNames[Opt::USER]).toUtf8().toStdString();

            if (parser.isSet(sOptionNames[Opt::PASS]))
                server->pass = parser.value(sOptionNames[Opt::PASS]).toUtf8().toStdString();
        ...
        while (!stream.atEnd())
        {
            ...
            else
            {
                ...
                if (args.size() >= 2)
                {
                    ...
                    else if (opt == sOptionNames[Opt::LOG_IN_FILE] && useHMI())
                    {
                        ...
                        if (val == "true" || val == "on" || val == "1")
                        {
                            ...
                            if (_logFile->open(QIODevice::WriteOnly|QIODevice::Text))
                            {
                                ...
                                QString logLine = tr("ngPost starts logging: %1").arg(QDateTime::currentDateTime().toString("yyyy/MM/dd hh:mm:ss"));
                                logLine.replace("ngPost", displayName());
                                _log(logLine);
                            }
                            else
                            {
                    ...
                    {
                        const QString lowerVal = val.toLower();
                        QStringList obfuscateTargets;
                        for (const QString &target : lowerVal.split(",", Qt::SkipEmptyParts))
                            obfuscateTargets << target.trimmed();
                        if (lowerVal.startsWith("article") || obfuscateTargets.contains("article"))
                        {
                            _obfuscateArticles = true;
                            qDebug() << "Do article obfuscation (the subject of each Article will be a UUID)\n";
                        }
                        if (obfuscateTargets.contains("file") || obfuscateTargets.contains("filename")
                                || obfuscateTargets.contains("file_name") || obfuscateTargets.contains("file-name"))
                            _obfuscateFileName = true;
                    }
                    else if (opt == sOptionNames[Opt::OBFUSCATE_FILE_NAME])
                    {
                        val = val.toLower();
                        if (val == "true" || val == "on" || val == "1")
                            _obfuscateFileName = true;
                    ...
                    else if (opt == sOptionNames[Opt::THEME_MODE])
                    {
                        const QString mode = val.trimmed().toLower();
                        if (mode == "dark")
                            _themeMode.clear();
                        else if (mode == "light")
                            _themeMode = mode;
                        else
                            err += tr("THEME_MODE should be DARK or LIGHT\n");
                    }

                    else if (opt == sOptionNames[Opt::THEME_COLOR])
                    {
                        QString color = val.trimmed();
                        if (color.startsWith('#'))
                            color = color.mid(1);
                        QRegularExpression re("^[0-9a-fA-F]{6}$");
                        if (re.match(color).hasMatch())
                            _themeColor = QString("#%1").arg(color.toUpper());
                        else
                            err += tr("THEME_COLOR should be a hex RGB value like #1E90FF\n");
                    }

                    else if (opt == sOptionNames[Opt::UI_SCALE])
                    {
                        double scaleValue = val.trimmed().replace(',', '.').toDouble(&ok);
                        if (ok)
                        {
                            if (scaleValue > 10.0)
                                scaleValue /= 100.0;
                            if (scaleValue >= 1.0 && scaleValue <= 1.5)
                                _uiScale = scaleValue;
                            else
                                err += tr("UI_SCALE should be between 100 and 150\n");
                        }
                        else
                            err += tr("UI_SCALE should be a number like 100 or 1.23\n");
                    }
                    else if (opt == sOptionNames[Opt::QUICK_ADVANCED_VISIBLE])
                    {
                        val = val.toLower();
                        if (val == "true" || val == "on" || val == "1")
                            _quickAdvancedVisible = true;
                    }
                    else if (opt == sOptionNames[Opt::AUTO_ADVANCED_VISIBLE])
                    {
                        val = val.toLower();
                        if (val == "true" || val == "on" || val == "1")
                            _autoAdvancedVisible = true;
                    }
                    else if (opt == sOptionNames[Opt::QUICK_POST_ARCHIVE_PASSWORD])
                    {
                        val = val.toLower();
                        if (val == "true" || val == "on" || val == "1")
                            _quickPostArchivePassword = true;
                    }
                    ...
                    else if (opt == sOptionNames[Opt::AUTO_COMPRESS])
                    {
                        if (useHMI())
                        {
                            _packAutoKeywords = defaultPackAutoKeywords();
                            enableAutoPacking();
                    ...
                    {
                        serverParams->user = val.toUtf8().toStdString();
                        serverParams->auth = !val.isEmpty() || !serverParams->pass.empty();
                    }
                    else if (opt == sOptionNames[Opt::PASS])
                    {
                        serverParams->pass = val.toUtf8().toStdString();
                        serverParams->auth = !val.isEmpty() || !serverParams->user.empty();
                    }
...
void NgPost::_syntax(char *appName)
{
    ...
    _cout << "\n" << tr("Examples:") << "\n"
          ...
          << "  - " << tr("with config file") << ": " << app << " -c /path/to/ngPost.conf -m \"password=qwerty42\" -f ngPost@nowhere.com -i /tmp/file1 -i /tmp/file2 -i /tmp/folderToPost1 -i /tmp/folderToPost2\n"
...
{
    const QString conf = _prepareDefaultConfigPath(true);
    if (conf.isEmpty())
        return QString();
...
void NgPost::saveConfig()
{
    ...
    if (conf.isEmpty())
        return;
    const QString bundledArchiver = _findBundledArchiver();
    const bool usingBundledArchiver = !bundledArchiver.isEmpty() && QFileInfo(_rarPath).absoluteFilePath() == QFileInfo(bundledArchiver).absoluteFilePath();
    const QString savedRarPath = usingBundledArchiver ? QString() : _rarPath;
    const QStringList savedPackAutoKeywords = _packAutoKeywords.isEmpty() ? defaultPackAutoKeywords() : _packAutoKeywords;

    if (!_ensureConfigDirectory(conf))
        return;
    ...
    if (file.open(QIODevice::WriteOnly|QIODevice::Text))
    {
        ...
        stream << tr("# ngPost configuration file") << "\n"
               ...
               << "#\n"
               << "\n"
               << tr("## Lang for the app. Currently supported: EN, DE, NL") << "\n"
               << "lang = " << _lang.toUpper() << "\n"
               << "\n"
               ...
               << "\n"
               << tr("## keep the advanced quick post options expanded in the GUI") << "\n"
               << (_quickAdvancedVisible ? "QUICK_ADVANCED_VISIBLE = true\n" : "#QUICK_ADVANCED_VISIBLE = true\n")
               << "\n"
               << tr("## keep the archive password option enabled by default in Quick Post") << "\n"
               << (_quickPostArchivePassword ? "QUICK_POST_ARCHIVE_PASSWORD = true\n" : "#QUICK_POST_ARCHIVE_PASSWORD = true\n")
               << "\n"
               << tr("## keep the advanced auto post options expanded in the GUI") << "\n"
               << (_autoAdvancedVisible ? "AUTO_ADVANCED_VISIBLE = true\n" : "#AUTO_ADVANCED_VISIBLE = true\n")
               << "\n"
        ...
        stream << "\n"
               ...
               << "\n"
               << tr("## rename the file stored inside the archive when using random archive names") << "\n"
               << (_obfuscateFileName ? "" : "#") << "OBFUSCATE_FILE_NAME = true\n"
               << "\n"
               ...
               << tr("#PACK = GEN_PAR2") << "\n"
               << (_packAuto ? QString("PACK = %1\n").arg(savedPackAutoKeywords.join(", ").toUpper()) : "")
               << "\n"
               ...
               << "RAR_PASS = "
#ifdef __USE_HMI__
               << ((_hmi && !_hmi->useFixedPassword()) ? "yourPassword" : (_rarPassFixed.isEmpty() ? "yourPassword" : _rarPassFixed))
#else
               << (_rarPassFixed.isEmpty()  ? "yourPassword" : _rarPassFixed)
#endif
               << "\n"
               << "\n"
        ...
        stream << tr("## RAR or 7zip absolute file path (external application)") << "\n"
               ...
               << tr("## ngPost first looks next to ngPost.exe for 7za, 7zz, 7zr, 7z or rar") << "\n"
               << tr("## leave this empty to use the bundled archiver automatically") << "\n"
               << "RAR_PATH = " << savedRarPath << "\n"
               << "\n"
        ...
        for (NntpServerParams *param : _nntpServers)
        {
            stream << "[server]\n"
                   ...
                   << "user = " << QString::fromUtf8(param->user.c_str()) << "\n"
                   << "pass = " << QString::fromUtf8(param->pass.c_str()) << "\n"
...
void NgPost::saveThemeColor()
{
    ...
    {
        _error(tr("The config file '%1' is not readable...").arg(conf));
        return;
    }
    ...
    const QString themeLine = _themeColor.isEmpty()
        ? QString("#THEME_COLOR = #1E90FF")
        : QString("THEME_COLOR = %1").arg(_themeColor);
    QRegularExpression themeRe("^\\s*#?\\s*THEME_COLOR\\s*=.*$", QRegularExpression::MultilineOption);
    if (themeRe.match(contents).hasMatch())
    {
        contents.replace(themeRe, themeLine);
    }
    else
    {
        QRegularExpression langRe("^\\s*lang\\s*=.*$", QRegularExpression::MultilineOption);
        QRegularExpressionMatch match = langRe.match(contents);
        const QString insertBlock = QString("\n\n## UI theme accent color (hex, e.g. #1E90FF)\n%1").arg(themeLine);
        if (match.hasMatch())
            contents.insert(match.capturedEnd(), insertBlock);
        else
            contents = QString("## UI theme accent color (hex, e.g. #1E90FF)\n%1\n\n").arg(themeLine) + contents;
    }

    _writeTextFileAtomically(conf, contents);
}

void NgPost::saveThemeMode()
{
    const QString conf = _prepareDefaultConfigPath(true);
    if (conf.isEmpty())
        return;
    ...
    const QString themeModeLine = _themeMode.isEmpty()
        ? QString("#THEME_MODE = LIGHT")
        : QString("THEME_MODE = %1").arg(_themeMode.toUpper());
    QRegularExpression themeModeRe("^\\s*#?\\s*THEME_MODE\\s*=.*$", QRegularExpression::MultilineOption);
    if (themeModeRe.match(contents).hasMatch())
    {
        contents.replace(themeModeRe, themeModeLine);
    }
    else
    {
        QRegularExpression langRe("^\\s*lang\\s*=.*$", QRegularExpression::MultilineOption);
        QRegularExpressionMatch match = langRe.match(contents);
        const QString insertBlock = QString("\n\n## UI theme mode (LIGHT or DARK in the current GUI)\n%1").arg(themeModeLine);
        if (match.hasMatch())
            contents.insert(match.capturedEnd(), insertBlock);
        else
            contents = QString("## UI theme mode (LIGHT or DARK in the current GUI)\n%1\n\n").arg(themeModeLine) + contents;
    }

    _writeTextFileAtomically(conf, contents);
}

void NgPost::saveUiScale()
{
    const QString conf = _prepareDefaultConfigPath(true);
    if (conf.isEmpty())
        return;
    ...
    const QString scaleLine = qFuzzyCompare(_uiScale, 1.0)
        ? QString("#UI_SCALE = 100")
        : QString("UI_SCALE = %1").arg(qRound(_uiScale * 100.0));
    QRegularExpression scaleRe("^\\s*#?\\s*UI_SCALE\\s*=.*$", QRegularExpression::MultilineOption);
    if (scaleRe.match(contents).hasMatch())
    {
        contents.replace(scaleRe, scaleLine);
    }
    else
    {
        QRegularExpression themeModeRe("^\\s*#?\\s*THEME_MODE\\s*=.*$", QRegularExpression::MultilineOption);
        QRegularExpressionMatch match = themeModeRe.match(contents);
        const QString insertBlock = QString("\n## UI scale percentage for the current GUI (100, 110, 123 or 150)\n%1").arg(scaleLine);
        if (match.hasMatch())
            contents.insert(match.capturedEnd(), insertBlock);
        else
            contents = QString("## UI scale percentage for the current GUI (100, 110, 123 or 150)\n%1\n\n").arg(scaleLine) + contents;
    }

    _writeTextFileAtomically(conf, contents);
}

QString NgPost::_defaultConfigDirectory() const
{
    if (_portableModeEnabled())
    ...
    {
#if defined(WIN32) || defined(__MINGW64__)
        dirPath = QDir::home().absoluteFilePath(QStringLiteral("AppData/Local/%1").arg(sAppName));
#else
        dirPath = QDir::home().absoluteFilePath(QStringLiteral(".config/%1").arg(sAppName));
#endif
    }
    return QDir::cleanPath(dirPath);
}

QString NgPost::_defaultConfigPath() const
{
    return QDir(_defaultConfigDirectory()).filePath(QString::fromLatin1(sDefaultConfig));
}

QString NgPost::_portableMarkerPath() const
{
    return QDir(QCoreApplication::applicationDirPath()).absoluteFilePath(QString::fromLatin1(sPortableMarker));
}

bool NgPost::_portableModeEnabled() const
{
    const QFileInfo markerInfo(_portableMarkerPath());
    return markerInfo.exists() && markerInfo.isFile();
}

QStringList NgPost::_legacyDefaultConfigPaths() const
{
    QStringList paths;
#if defined(WIN32) || defined(__MINGW64__)
    paths << QDir(QCoreApplication::applicationDirPath()).absoluteFilePath(QString::fromLatin1(sDefaultConfig));
    const QString currentPath = QDir::current().absoluteFilePath(QString::fromLatin1(sDefaultConfig));
    ...
    if (_portableModeEnabled())
    {
        ...
        if (appDataDir.isEmpty())
            appDataDir = QDir::home().absoluteFilePath(QStringLiteral("AppData/Local/%1").arg(sAppName));
        const QString appDataPath = QDir(appDataDir).absoluteFilePath(QString::fromLatin1(sDefaultConfig));
        if (!paths.contains(appDataPath))
            paths << appDataPath;
    }
#else
    paths << QDir::home().absoluteFilePath(QString::fromLatin1(sDefaultConfig));
#endif
    return paths;
}

bool NgPost::_ensureConfigDirectory(const QString &configPath) const
{
    const QString dirPath = QFileInfo(configPath).absolutePath();
    if (dirPath.isEmpty())
        return true;
    ...
    _error(tr("Error: Couldn't create configuration directory: %1").arg(dirPath));
    return false;
}

QString NgPost::_prepareDefaultConfigPath(bool migrateLegacy)
{
    const QString conf = _defaultConfigPath();
    if (!_ensureConfigDirectory(conf))
        return QString();

    if (!migrateLegacy || QFileInfo(conf).exists())
        return conf;

    for (const QString &legacyPath : _legacyDefaultConfigPaths())
    {
        QFileInfo legacyInfo(legacyPath);
        if (!legacyInfo.exists() || !legacyInfo.isFile() || !legacyInfo.isReadable())
            continue;

        if (legacyInfo.absoluteFilePath() == QFileInfo(conf).absoluteFilePath())
            return conf;

        if (QFile::copy(legacyInfo.absoluteFilePath(), conf))
        {
            if (!_quiet)
                _log(tr("Migrated configuration from '%1' to '%2'").arg(legacyInfo.absoluteFilePath(), conf));
            return conf;
        }
    }
...
bool NgPost::_writeTextFileAtomically(const QString &configPath, const QString &contents)
{
    if (!_ensureConfigDirectory(configPath))
        return false;

    QSaveFile file(configPath);
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text))
    {
        _error(tr("Error: Couldn't write default configuration file: %1").arg(configPath));
        return false;
    }
