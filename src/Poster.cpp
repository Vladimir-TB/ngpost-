...
bool Poster::prepareArticlesInAdvance()
{
    ...
    for (int i = 0; i < nbArticlesToPrepare ; ++i)
    {
        ...
        {
#ifdef __DEBUG__
            _job->_log(QString("[%1] prepareArticlesInAdvance : no more Articles to produce after i = %2").arg(_builderThread.objectName()).arg(i));
#endif
            canProduceAll = false;
            break;
        }
...
#ifdef __DEBUG__
    _job->_log(QString("[%1] prepareArticlesInAdvance: Article queue size:  %2").arg(_builderThread.objectName()).arg(_articles.size()));
#endif
