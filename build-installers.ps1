...
$installer = Join-Path $out 'ngPost-setup-v5.1.2-UNSIGNED.exe'
if (-not (Test-Path -LiteralPath $installer)) { throw 'Expected installer missing' }
$portable = Join-Path $out 'ngPost-portable-v5.1.2-UNSIGNED.zip'
...
$guide = @(
    '# ngPost+ 5.1.2 - UNSIGNED',
    '',
    'Windows x64-release. Installer en app zijn niet digitaal ondertekend.',
    '',
    '- Installeren: ngPost-setup-v5.1.2-UNSIGNED.exe (standaard Engels; Nederlands selecteerbaar).',
    '- Portable: pak ngPost-portable-v5.1.2-UNSIGNED.zip helemaal uit en start ngPost.exe.',
    ...
    '- Raadpleeg ../../STATUS.md en ../../docs/LEGAL-REVIEW.md voor controlebewijs en juridische achtergrond.'
)
